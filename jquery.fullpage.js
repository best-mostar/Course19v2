...
var homeItems = document.getElementsByClassName("homeList");
	...
	var Fullpage = (function () {
		...
		Fullpage.prototype = {
			...
			bindEvent: function () {
				...
				$('.sticky li').on('click', function (e){
					var crrId = $(this).attr('id');
					that.gotoTarget(crrId - 1);
				});
			},
			gotoTarget: function (targetIndex) {
				...
				for (let index = 0; index < homeItems.length; index++) {
					...
					homeItems[index].classList.remove("selected");
				}
				homeItems[targetIndex].classList.add("selected");
